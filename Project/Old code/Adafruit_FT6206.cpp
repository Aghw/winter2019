...
void Adafruit_FT6206::setPjdfHandle(HANDLE hIC2) {
    this->hI2C = hIC2;
}
...
//-----------------------------------------------------------------
// This function has to be modified to use PFDJ driver functions
// don't call I2C_* functions from here anymore
// instead move the I2C_* function calls to pjdfInternalI2C.c
//-----------------------------------------------------------------
void Adafruit_FT6206::readData(uint16_t *x, uint16_t *y) {
  ...
  hI2C = Open(PJDF_DEVICE_ID_I2C1, I2C_Direction_Transmitter);
//  err = Write(hI2C, &touches, 0);
  err = Read(hI2C, &touches,  0);
//  err = Close(hI2C);
//  hI2C = Open(PJDF_DEVICE_ID_I2C1, I2C_Direction_Receiver);
...
//  err = ReadI2C(I2C1, (uint8_t)0);
//  err = I2C_Driver->read(I2C1, (uint8_t)0);
...
uint8_t Adafruit_FT6206::readRegister8(uint8_t reg) {
    uint8_t x = 0;
    PjdfErrCode err = 0;
//    Ioctl(hI2C, PJDF_CTRL_I2C_SET_DEVICE_ADDRESS, 0, 0);
    
    hI2C = Open(PJDF_DEVICE_ID_I2C1, I2C_Direction_Transmitter);
//    err = Write(hI2C, &reg,  0);
    err = Read(hI2C, &reg,  0);
//    err = Close(hI2C);
...
void Adafruit_FT6206::writeRegister8(uint8_t reg, uint8_t val) {
   // use i2c
    ...
    hI2C = Open(PJDF_DEVICE_ID_I2C1, 0);
    err = Write(hI2C, &reg,  0);
    err = Write(hI2C, &val,  0);
    err = Close(hI2C);
//    Ioctl(hI2C, PJDF_CTRL_I2C_SET_DEVICE_ADDRESS, 0, 0);
}
